...
///////////////////////////    selectPairs.C    //////////////////////////////
////                                                                      ////
////    Input : makePairs.C output trees containing pre-selected pairs    ////
////                                                                      ////
////    - further selection of the pairs                                  ////
////      --> tag : VBTF WP80 + eT > 20 GeV                               ////
////      + trig EG12/15 (HLT selection's L1 seed early 2011 SingleEle)   ////
////      --> probe : VBTF WP95 + eT > 5 GeV                              ////
////      --> mass : between 60 GeV and 120 GeV                           ////
////    - if pair selected : check if probe trig EGn                      ////
////                                                                      ////
////    Output : - trees readable by fastEfficiencyNadir.C to             ////
////               plot the efficiency turn-on curve                      ////
////             - counters of electrons / matched / matchedM etc         ////
////                                                                      ////
////   Designed to printout all the information for inefficient electrons ////
////                                                                      ////
////   Adding EG18 and EG22 (HLT menu 3e33)                               ////
////   + map efficiency eta/phi                                           ////
///////////////////////////    !!! ENJOY IT ;-) !!!    ///////////////////////
//////////////////////////////////////////////////////////////////////////////

// 1.1 : correcting Tag&Probe method 
// 1.2 : 
// 1.3 : adding ele_RCTL1iso_To --> 3 sets of ranks : data, data To, M (M To)
// 1.3.1 : correcting names iColl

#include "../Common/baseFuncNad.1.5.h"

void process(TChain* myChain, TString dirOut, int iEff, int iEff_M,
  ...
  // LOG FOR PROBES RUN:EVENT //
  ofstream out_elepairs_json(dirOut+"elepairs.json",ios::out);
  ofstream outNotToZero(dirOut+"notToZero.txt", ios::out);
  ofstream out_probes_trigEG15(dirOut+"probes_trigEG15.txt", ios::out);
  ofstream out_probes_notrigEG15(dirOut+"probes_notrigEG15.txt", ios::out);
  ofstream out_notrigEG15_RLE(dirOut+"probes_notrigEG15_RLE.txt",ios::out);
  ofstream out_notrigEG15_R(dirOut+"probes_notrigEG15_R.txt",ios::out);
  ofstream out_custom_json(dirOut+"inefficiency.json",ios::out);
  ...
  TString name_coll[nColl] = {"_N","_To","_M"};
  ...
  int trig_tower_N_E,trig_tower_ieta_E[4032],trig_tower_iphi_E[4032],trig_tower_adc_E[4032][5],trig_tower_sFGVB_E[4032][5]; 

  trig_tower_N = trig_tower_N_modif = trig_tower_N_E = 0;
  for(int iT=0;iT<4032;iT++) {
    ...
    trig_tower_ieta_E[iT]=trig_tower_iphi_E[iT]=-999;
    for(int iEm=0;iEm<5;iEm++)
      trig_tower_adc_E[iT][iEm]=trig_tower_sFGVB_E[iT][iEm]=-999;
  }
  ...
  int pair_cuts[2], pair_HLT_Ele27_cut[2], pair_fidu[2], pair_charge[2], pair_RCTeta[2], pair_RCTphi[2], 
    pair_L1iso[2], pair_L1noniso[2], pair_L1iso_To[2], pair_L1noniso_To[2], pair_L1iso_M[2], pair_L1noniso_M[2];
  int pair_RCTetaVect[2][10], pair_RCTphiVect[2][10], 
    pair_L1isoVect[2][10], pair_L1nonisoVect[2][10], 
    pair_L1isoVect_M[2][10], pair_L1nonisoVect_M[2][10], pair_L1isoVect_To[2][10], pair_L1nonisoVect_To[2][10];
...
//   }
  ...
  myChain->SetBranchAddress("pair_L1iso_To",&pair_L1iso_To);
  myChain->SetBranchAddress("pair_L1noniso_To",&pair_L1noniso_To);
  ...
  myChain->SetBranchAddress("pair_L1isoVect_To",&pair_L1isoVect_To);
  myChain->SetBranchAddress("pair_L1nonisoVect_To",&pair_L1nonisoVect_To);
  ...
  int sFGVB_M[5], adc_E[5];

  // TRIGGERING PROPERTIES //
  vector<int> trigHLT;
  vector<int> firedEG[nColl][2]; // [N/To/M][tag/probe]
  int iTrig[nColl][2]; // [N/To/M][tag/probe]
  int tagEG;
  ...
  if(nEntries>=0 && nEntries<numEntries)
    nProcess = nEntries;
  outlog << "will process " << nProcess << "/" << numEntries << " pairs" << endl;
  ...
  for (int iPair = 0 ; iPair < nProcess ; iPair++ ) { 
    ...
    // ORIGINALLY (ONLINE) INEFF/EFF PROBES : EVENT SELECTION //

//     isGoodEvent = false;

//     if(period=="Apr22") {
//       //if(iEff==0)
//       isGoodEvent = isGoodEvent_ineff(nEvent);
//       //if(iEff==1)
//       //isGoodEvent = isGoodEvent_eff(nEvent);
//     }
//     /*
//     else if(period=="May10") {
//       if(iEff==0)
// 	isGoodEvent = isGoodEvent_ineff_May10(nEvent);
//       if(iEff==1)
// 	isGoodEvent = isGoodEvent_eff_May10(nEvent);
//     }
//     */
//     if( !isGoodEvent ) {
//       //outlog << "no good event" << endl;
//       //continue;
//     }
//     if(debug) cout << "passed JSON" << endl;
	...
	globalFireL1_Normal( pair_RCTetVect[iEle][iR] , pair_L1nonisoVect[iEle][iR],
			     pair_L1isoVect[iEle][iR], firedEG[0][iEle], menu); 
	
	globalFireL1_Normal( pair_RCTetVect[iEle][iR] , pair_L1nonisoVect_To[iEle][iR],
			     pair_L1isoVect_To[iEle][iR], firedEG[1][iEle], menu); 
	
	globalFireL1_Normal( pair_RCTetVect[iEle][iR] , pair_L1nonisoVect_M[iEle][iR],
			     pair_L1isoVect_M[iEle][iR], firedEG[2][iEle], menu); 
...
//     }
	...
	if( pair_eta[iProbe]<1.479 && pair_eta[iTag]<1.479 ) h_Mee[0]->Fill(pair_M);
	if( pair_eta[iProbe]>1.479 && pair_eta[iTag]>1.479 ) h_Mee[1]->Fill(pair_M);
	...
	if( pair_sclEt[iProbe]>15. && firedEG[0][iProbe][7]==iEff && firedEG[1][iProbe][7]==iEff_M) {
	  ...
	  if( iPair==0 || nEvent!=nCurrentEvt ) {
	    ...
	    for(int iL1=0 ; iL1<4 ; iL1++) {
	      outlog << "(" << trig_L1emIso_ieta[iL1]      << ";" << trig_L1emIso_iphi[iL1]      << ")=" << trig_L1emIso_rank[iL1]      << "      "
		     ...
		     << "(" << trig_L1emIso_ieta_M_To[iL1]    << ";" << trig_L1emIso_iphi_M_To[iL1]    << ")=" << trig_L1emIso_rank_M_To[iL1]    << "      "
		     << "(" << trig_L1emNonIso_ieta_M_To[iL1] << ";" << trig_L1emNonIso_iphi_M_To[iL1] << ")=" << trig_L1emNonIso_rank_M_To[iL1] << "      ";
	  ...
	  // TP INFORMATION  
	  outlog << "TRIGGER PRIMITIVES" << endl;  
	  for(int iRCTeta=4 ; iRCTeta<=17 ; iRCTeta++) {
		...
		for (int j=0 ; j<(int)iphiTT.size() ; j++) {
		  ...
		  tOri = adcTT[coords].first;
		  tM = adcTT[coords].second;
		  adc = trig_tower_adc[tOri] ;
		  sFGVB = trig_tower_sFGVB[tOri] ;
		  for(int iEm=0 ; iEm<5 ; iEm++) {
		    adc_E[iEm] = trig_tower_adc_E[tM][iEm] ;
		    sFGVB_M[iEm] = trig_tower_sFGVB_E[tM][iEm];
		  }
		  // OUTPUT : TT
		  if( (adc!=0 || adc_E[0]!=0 || adc_E[1]!=0 || adc_E[2]!=0 || adc_E[3]!=0 || adc_E[4]!=0) 
		      && (adc_E[0]!=-999 && adc_E[1]!=-999 && adc_E[2]!=-999 && adc_E[3]!=-999 && adc_E[4]!=-999) 
		      || (adc==0 && (adc_E[0]==-999 || adc_E[1]==-999 || adc_E[2]==-999 || adc_E[3]==-999 || adc_E[4]==-999))
		      ) {
		    outlog << "Reta="    << iRCTeta 
			   << " Rphi="   << iRCTphi   
			   << " Teta=" << ietaTT[i] 
		    ...
		    // tell the zeroing status of the TP ////////////
		    if( iRCTeta==pair_RCTeta[ IdxTagProbe[1] ] && // only for TP in the probe RCT region
			iRCTphi==pair_RCTphi[ IdxTagProbe[1] ] ) {
		      if( isInEB( iRCTeta, ietaTT[i] ) ) { // EB case

			b_notToZero = false;
		      
			for(int iEm=0 ; iEm<5 ; iEm++) {
		     
			  // Impact of the Spike Cleaning
			  if( adc_E[iEm]>16 && sFGVB_M[iEm]==0 ) {
			    outlog << " ToBeZeroed" ;
			    nZeroStatus[0]++ ;
			    if( adc==0 ) {
			      outlog << " Zeroed" ;
			  ...
			  // Zeroed not by the Spike Cleaning
			  else if( adc_E[iEm]>6 && sFGVB_M[iEm]==1 ) {
			    if( adc<=2 ) {
			      outlog << " | EBTT Lost " << adc_E[iEm]-adc ;
			      if( adc==0 ) outlog << " lostAll" ;
			      nEB_NotToZero_Zeroed++ ;
			      b_notToZero = true;
			    }
			  }
			  // zeroed not by the spike cleaning
			}
			// record apart the TP EBTT notToZero Zeroed
			if( b_notToZero ) {
			  outNotToZero << "Reta="    << iRCTeta 
				       << " Rphi="   << iRCTphi   
				       << " Teta=" << ietaTT[i] 
				       << " Tphi=" << iphiTT[j] 
				       << " adc="       << adc       
				       ...
				       << " adc_M=[" ;
			  for(int iEm=0 ; iEm<5 ; iEm++) {
			    outNotToZero << adc_E[iEm] << "," ;
			  }
			  outNotToZero << "] sFGVB_M=[" ;
			  for(int iEm=0 ; iEm<5 ; iEm++) {
			    outNotToZero << sFGVB_M[iEm] ;
			    if(iEm<4) outNotToZero << "," ;
			  }
			  outNotToZero << "] EBTT NotToZero" << endl;
			}
		      }
			...
			for(int iEm=0 ; iEm<5 ; iEm++) {
			  if( adc_E[iEm]>6 ) {
			    if( adc<=2 ) {
			      outlog << " | EETT Lost " << adc_E[iEm]-adc ;
			      nEE_NotToZero_Zeroed++ ;
			      b_notToZero = true;
			    }
			  }
			}
			...
			if( b_notToZero ) {
			  ...
			  outNotToZero << "] EETT NotToZero" << endl;
			}
	      ...
	      }// loop over TTs of a given region
	    }
	  }// loop over RCT regions
	  outlog << endl 
		 << "------------------------------------------------------------------------------------------------"
		 << endl;
  ...
  for(int i=0 ; i<3 ; i++) {
    ...
    c_Mee[i]->Print(dirOut+"mass_Zee_"+nameC[i]+".gif","gif");
  }
  cout << "...plotted !" << endl;
  ...
  */
  ...
  for(int iC=0 ; iC<3 ; iC++) {
    ...
    h_Map[iC]->Draw("colz");
    c_Maps[iC]->Print(dirOut+"maps"+name_c_Maps[iC]+".gif","gif");
  }
  ...
  for(int iC=0 ; iC<3 ; iC++) {
    ...
    h_Map1D[iC]->Draw("");
    c_Maps1D[iC]->Print(dirOut+"maps"+name_c_Maps1D[iC]+".gif","gif");
  }
  cout << "plotted" << endl;
  ...
  for(int iEG=15 ; iEG<16 ; iEG++) {
    g_effi_map[iEG] = new TGraph2D("g_effi_map_EG"+EGval[iEG],"Efficiency Eta/Phi Map : EG"+EGval[iEG],10000,bin_eta,bin_phi,effi_val);
    
    c_effi_map[iEG] = new TCanvas("c_effi_map_EG"+EGval[iEG],"Efficiency Eta/Phi Map : EG"+EGval[iEG],0,0,800,600);
    gStyle->SetPalette(1);
    g_effi_map[iEG]->Draw("colz");
    c_effi_map[iEG]->Print(dirOut+"g_effi_map_EG"+EGval[iEG]+".gif");
    //c_effi_map[iEG]->Print(dirOut+"g_effi_map_EG"+EGval[iEG]+".C");
  }

  // OUTPUT LOGS //
  if(debug) cout << "Filling output logs..." << endl;
  ofstream outCounters(dirOut+"counters_selectPairs.csv", ios::out);
  ...
  for(int iECAL=0 ; iECAL<nECAL ; iECAL++ ) {
    ...
    outCounters << " | " ;
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outCounters << nMatched_M[iPt][iEG] << "," ;
    }
    
    outCounters << " | " ;
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outCounters << nLost[iPt][iEG] << "," ;
      }
    outCounters << " | " ;
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outCounters << nAppeared[iPt][iEG] << "," ;
      }
    */
    outCounters << endl;
  }
...
void selectPairs(int nEntries=-1, int iEff=0, int iEff_M=0,
		 TString dirOut="",
		 TString dirIn="",
...
{
  // Output Log
  ofstream outlog(dirOut+"log_selectPairs.txt",ios::out);

  // Input trees
  TChain * myChain = new TChain(nameChain);

  myChain->Add(dirIn+"/*.root");
  ...
  */
  ...
  process(myChain, dirOut, iEff, iEff_M, tagCuts, tagEt, probeCuts, probeEt, massCut1, massCut2,
	  nEntries, outlog, tag_hlt, checkCharge, data2011, debug);
