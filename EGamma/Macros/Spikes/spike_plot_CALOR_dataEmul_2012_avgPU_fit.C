...
float errPoissCount(float n, float ntot) {
  
  if(ntot==0) {
    cout << "errPoissCount ERROR : ntot=0" << endl;
    return 0;
  }
  ...
  if( x<0 ) {
    cout << "errPoissCount ERROR : sqrt(<0)" << endl;
    return 0;
  }

  return (1/ntot)*sqrt(n*(1-n/ntot)) ;
}

float errPoissCount_Alex(float n, float ntot) {
...
void spike_summarize(TString dirOut="./HPU2012/Rebin/rebin4/", int factor=60, TString graph_style="A*") {
  ...
  */
  ...
  const int nSrc=6; // data 2011B, newkill 2011B, data highPU, emul highPU, 2012 online HPU
  const int nStrict = 2;
  const int nEG = 8;
  const int nVtx = 40;
  const int nVTX[nSrc]={40,40,40,40,60,60};
  const int nEG_g=4; // nEG for graph : EG12,15,20,30

  //TString src_name[nSrc]={"2011A","2011B","highPU"};
  TString src_name[nSrc]={"data_2011B","newsetting_2011B", "data_highPU", "newsetting_HPU", "d2012_HPU_198603", "d2012_HPU_198609"};
  ...
  file[4]="/home/llr/cms/ndaci/SKWork/macro/skEfficiency/Spikes2012/HPU/Run198603/spike_plots_half_0_1.root" ;
  file[5]="/home/llr/cms/ndaci/SKWork/macro/skEfficiency/Spikes2012/HPU/Run198609/spike_plots_half_0_1.root" ;
  ...
  for(int iSrc=0 ; iSrc<nSrc ; iSrc++) {
    ...
    for( int iEG=0 ; iEG<nEG ; iEG++ ) {
      ...
      if(iSrc>=4) {
	cout << "### (any) nBins before rebin = " << h_evts_trigBy_any[iEG][iSrc]->GetNbinsX() << endl;
	h_evts_trigBy_any[iEG][iSrc] -> Rebin(60);
	cout << "### (any) nBins after rebin = "  << h_evts_trigBy_any[iEG][iSrc]->GetNbinsX() << endl;
      }
	...
	if(iSrc>=4) {
	  cout << "### (spikes) nBins before rebin = " << h_evts_trigBy_spikes[iEG][iStrict][iSrc]->GetNbinsX() << endl;
	  h_evts_trigBy_spikes[iEG][iStrict][iSrc] -> Rebin(60);
	  cout << "### (spikes) nBins after rebin = "  << h_evts_trigBy_spikes[iEG][iStrict][iSrc]->GetNbinsX() << endl;
	}
	cout << "got histogram "+ name_histo <<endl;
      }
    }
  ...
  const int nBins_src[nSrc][nEG_g]={ {21,15,19,16} , {16,15,13,10} , {28,25,24,21} , {28,25,24,21} , {1,1,1,1} , {1,1,1,1}};

  int idxBinsFr[nSrc][nEG_g][nFr]={ { {2,22},{1,15},{2,20},{3,18} },     // 2011 B data
				    { {3,18},{1,15},{4,16},{5,14} },     // 2011 B emul
				    { {12,39},{14,38},{14,37},{14,34} }, // 2011 HPU data
				    { {12,39},{14,38},{14,37},{14,34} }, // 2011 HPU emul
				    //{ {30,60},{30,60},{30,60},{30,60} } };
				    { {1,1},{1,1},{1,1},{1,1} } ,        // 2012 HPU data
				    { {1,1},{1,1},{1,1},{1,1} } };       // 2012 HPU data
  
  int fact[nSrc]={1,1,1,1,60,60};
  ...
  int mark_style[nSrc] = {22,22,21,21,20,20};
  //int mark_col[nSrc] = {4,2,4,2};
  int mark_col[nSrc] = {kRed,kGreen+2,kRed,kGreen+2,kBlue,kViolet};
  ...
  for(int iSrc=0 ; iSrc<nSrc ; iSrc++) {
	...
	// peak/average of distribution of Nvtx
	//if(iSrc==4) { x_vtx[0]=33; err_vtx[0]=10;}
	//if(iSrc==5) { x_vtx[0]=42; err_vtx[0]=10;}
	if(iSrc==4) { x_vtx[0]=44; err_vtx[0]=10;}
	if(iSrc==5) { x_vtx[0]=66; err_vtx[0]=10;}
  ...
  */
  ...
  for(int iEG=1 ; iEG<3 ; iEG++) {
    for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {
      ...
      g_evt_contam_err[iEG][iStrict][0]->GetXaxis()->Set(70,0,70);
      ...
      g_evt_contam_err[iEG][iStrict][5]->Draw("P");
      ...
      leg->AddEntry(g_evt_contam[iEG][iStrict][0],"Run 2011B Data","P");
      leg->AddEntry(g_evt_contam[iEG][iStrict][1],"Run 2011B Emul new setting (data)","P");
      leg->AddEntry(g_evt_contam[iEG][iStrict][2],"High PU 2011 Data","P");
      leg->AddEntry(g_evt_contam[iEG][iStrict][3],"High PU 2011 Emul new setting (data)","P");
      leg->AddEntry(g_evt_contam[iEG][iStrict][4],"High PU 2012 (run 198603)","P");
      leg->AddEntry(g_evt_contam[iEG][iStrict][5],"High PU 2012 (run 198609)","P");
      leg->Draw();
...
//       c_evt_contam_err[iEG][iStrict]->Print(dirOut+"evt_contam_err_EG"+trigname[iEG+4]+"_"+strict_name[iStrict]+".pdf");
  ...
  const int nPts_merge=36;
  float x_vtx_merge[nPts_merge]; // Nvtx                 = 4-16(2011B emul) + 17-37(2011 HPU emul) + 44(2012 HPU data 1) + 66(2012 HPU data 2)
  for(int i=0 ; i<34 ; i++) {  // idx in x/y_vtx_merge = 0-12               13-33                  34                    35
    x_vtx_merge[i]=i+4;        // idx in y_vtx_src     = 0-12               3-23                    0                     0
  }
  x_vtx_merge[34]=44;
  x_vtx_merge[35]=66;

  // Merge graphs to perform a fit
  float y_merge[nPts_merge];
  float err_merge[nPts_merge];
  float err_vtx_merge[nPts_merge];

  for(int iX=0 ; iX<=12 ; iX++) {
    y_merge[iX]   = n_contam[2][0][1][iX];
    err_merge[iX] = err_contam[2][0][1][iX];
  }
  for(int iX=3 ; iX<=23 ; iX++) {
    y_merge[iX-3+13]   = n_contam[2][0][3][iX];
    err_merge[iX-3+13] = err_contam[2][0][3][iX];
  }

  y_merge[34]   = n_contam[2][0][4][0];
  err_merge[34] = err_contam[2][0][4][0];

  y_merge[35]   = n_contam[2][0][5][0];
  err_merge[35] = err_contam[2][0][5][0];

  for(int i=0 ; i<34 ; i++)
    err_vtx_merge[i]=0.49;

  err_vtx_merge[34]=10;
  err_vtx_merge[35]=10;

  TGraphErrors* g_merge = new TGraphErrors( nPts_merge, x_vtx_merge, y_merge, err_vtx_merge, err_merge );
  
  TCanvas *c_merge = new TCanvas("merge", "merge", 0,0,800,600);

  g_merge->SetMinimum(0);
  g_merge->SetMaximum(0.5);
  g_merge->SetMarkerColor(kGreen+2);
  g_merge->Draw("AP");

  TF1 *f_linear = new TF1("f_linear","[0]*x+[1]",0,80);
  TF1 *f_expo   = new TF1("f_expo"  ,"[0]*TMath::Exp([1]*x)+[2]",0,80);

  f_expo->SetParameters();
  f_expo->SetParLimits ();

  //g_merge->Fit("[0]*expo(-[1]*x)");
  //g_merge->Fit("[0]*x+[1]");

  //g_merge->Fit(f_linear,"R");
  //g_merge->Fit(f_expo,"R");

  //c_merge->Print(dirOut+"g_merge.C");
  c_merge->Print(dirOut+"g_merge.gif");
  c_merge->Print(dirOut+"g_merge.eps");
  // THE END //
}
