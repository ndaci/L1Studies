...
//   myChain->SetBranchAddress("trig_iMaskedRCTphi", &trig_iMaskedRCTphi);
  ...
  TH1F * h_spiky_adc[nEG][2]; // adc data/emul
  TString name_data[2] = {"data","emul"};
  TH1F * h_spike_eT[nEG];
  ...
  for( int i=0 ; i<nEG ; i++ ) {
    ...
    h_spike_eT[i]  = new TH1F("h_spike_eT_EG"+trigname[i],"h_spike_eT_EG"+trigname[i],200,0,200);
    for(int iData=0 ; iData<2 ; iData++)
      h_spiky_adc[i][iData] 
	= new TH1F("h_spiky_adc_EG"+trigname[i]+"_"+name_data[iData],"h_spiky_adc_EG"+trigname[i]+"_"+name_data[iData],128,0,128);
...
// 	      outlog << endl;
		...
		if( triggEG[itg] ) {
		  ...
		  h_spike_eT[itg]->Fill(spike_Et[irh]);						
		  h_spiky_adc[itg][0]->Fill(adcTT[coords].first.first);
		  h_spiky_adc[itg][1]->Fill(adcTT[coords].first.second);
...
// 	      }
  ...
  /*
    ...
    h_spiky_adc[itg][0]->Fill(adcTT[coords].first.first);
  */
  TCanvas * c_spike_eT[nEG];
  TCanvas * c_spiky_adc[nEG][2];
  ...
  for(int iEG=0 ; iEG<nEG ; iEG++) {
    ...
    c_spike_eT[iEG] = new TCanvas("c_spike_eT_EG"+trigname[iEG],"Spike eT (EG"+trigname[iEG]+")",0,0,800,600);
    h_spike_eT[iEG]->Draw();
    c_spike_eT[iEG]->Print(dirOut+"spike_eT_EG"+trigname[iEG]+".pdf","pdf");

    for(int iData=0 ; iData<2 ; iData++) {
      c_spiky_adc[iEG][iData] = 
	new TCanvas("c_spiky_adc_EG"+trigname[iEG],"Spiky trigger tower rank (EG"+trigname[iEG]+","+name_data[iData]+")",0,0,800,600);
      h_spiky_adc[iEG][iData]->Draw();
      c_spiky_adc[iEG][iData]->Print(dirOut+"spiky_tpg_EG"+trigname[iEG]+".pdf","pdf");
    }
	   ...
	   TString graph_style="A*", int marker_style=1, TString data="Spikes_Run2011A", TString data_sum="2011A", float spikeCut=8., bool debug=false)
{
  ...
  if( data=="HighPU_Newkill" ) {
    listTrees = list_Spikes_HighPU_Newkill();
  }
  else if( data=="Run2011A_EmulReco" ) {
