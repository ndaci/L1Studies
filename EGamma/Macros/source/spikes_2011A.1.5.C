...
void TreeToHistos(TChain* myChain, TString data, TString dirOut, ofstream& outlog, int nEntries, 
		  TString halftag, bool spikeCut, bool debug, TString graph_style, int sev_user, TString hlt_sel)  {
...
//   myChain->SetBranchAddress("trig_iMaskedRCTphi", &trig_iMaskedRCTphi);
  ...
  jsonFile1 = jsonFile2 = "/data_CMS/cms/ndaci/ndaci_2011A/JSON/Cert_160404-177515_7TeV_PromptReco_Collisions11_JSON.txt" ;
...
// 	      }
	...
	if( anyTrigEG[iEG] ) h_evts_trigBy_any[iEG]->Fill(vtx_N);
	...
	if( spikyL1 ) {
	  ...
	  if( !notOnlySpikyL1 ) h_evts_trigBy_spikes[iEG][1]->Fill(vtx_N);
	}
...
int spikes(int nEntries=-1, int iHalf=0, int nHalf=1, int sev_user=3, TString hlt_sel="",
	   ...
	   TString graph_style="A*", int marker_style=1, TString data="Spikes_Run2011A", bool spikeCut=false, bool debug=false)
{
  ...
  TreeToHistos(myChain, data, dirOut, outlog, nEntries, halftag, spikeCut, debug, graph_style,sev_user,hlt_sel);
