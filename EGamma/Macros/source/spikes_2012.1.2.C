#include "/home/llr/cms/ndaci/SKWork/macro/skEfficiency/analyzers/COMMON/baseFuncNad.h"

// tree lists
#include "/data_CMS/cms/ndaci/ndaci_2012/SpikesVtx/treelist.h"
...
void TreeToHistos(TChain* myChain, TString data_sum, TString dirOut, ofstream& outlog, int nEntries, 
		  TString halftag, float spikeCut, bool debug, TString graph_style, int sev_user, TString hlt_sel, int run_sel)  {
  ...
  int trig_tower_N_M,trig_tower_ieta_M[4032],trig_tower_iphi_M[4032],trig_tower_adc_M[4032],trig_tower_sFGVB_M[4032]; 
...
//   myChain->SetBranchAddress("trig_iMaskedRCTphi", &trig_iMaskedRCTphi);
  ...
  TH1F * h_evts_trigBy_any[nEG];
  TH1F * h_evts_trigBy_spikes[nEG][nStrict];
  ...
  for( int i=0 ; i<nEG ; i++ ) {
    ...
    name_histo = "h_evts_trigBy_any_EG"+trigname[i];
    h_evts_trigBy_any[i] = new TH1F(name_histo,name_histo,nVtx,0,nVtx);
    
    for( int iStrict=0 ; iStrict<nStrict ; iStrict++ ) {
      name_histo = "h_evts_trigBy_spikes_EG"+trigname[i]+"_"+strict_name[iStrict];
      h_evts_trigBy_spikes[i][iStrict] = new TH1F(name_histo,name_histo,nVtx,0,nVtx);
    }
  }
  ...
  for (int iEvent = 0 ; iEvent < nProcess ; iEvent++ )
	...
	if( iterTT != adcTT.end() ) {
	  adcTT[coords].first.second = trig_tower_adc_E[t][2];
	  adcTT[coords].second.second = trig_tower_sFGVB_E[t][2];
	  if(trig_tower_ieta_E[t]>-17 && trig_tower_ieta_E[t]<18) 
	    for(int i=0 ; i<IdxCat.size() ; i++)
	      h_adc[3]->Fill(0.25*trig_tower_adc_E[t][2]);
	      //ttfile << "etaM="    << trig_tower_ieta_M[t]
	      // << "   phiM=" << trig_tower_iphi_M[t]
	      //<< "   sFGVBM=" << trig_tower_sFGVB_M[t]
	      //<< endl;
	}
	else {
...
//       for(int t=0 ; t<trig_tower_N_M ; t++) {
// 	coords = make_pair( trig_tower_ieta_M[t] , trig_tower_iphi_M[t] );
// 	iterTT = adcTT.find( coords );
// 	if( iterTT != adcTT.end() ) {
// 	  adcTT[coords].first.second = trig_tower_adc_M[t];
// 	  adcTT[coords].second.second = trig_tower_sFGVB_M[t];
// 	  if(trig_tower_ieta_M[t]>-17 && trig_tower_ieta_M[t]<18) 
// 	    for(int i=0 ; i<IdxCat.size() ; i++)
// 	      h_adc[3]->Fill(0.25*trig_tower_adc_M[t]);
// 	  /*ttfile << "etaM="    << trig_tower_ieta_M[t]
// 		 << "   phiM=" << trig_tower_iphi_M[t]
// 		 << "   sFGVBM=" << trig_tower_sFGVB_M[t]
// 		 << endl;*/
...
// 	      }
	...
	if( anyTrigEG[iEG] ) {
	  h_evts_trigBy_any[iEG]->Fill(vtx_N);
  ...
  float n_evts_trigBy_any[nEG][nVtx+1];
  float n_evts_trigBy_spikes[nEG][nStrict][nVtx+1];
  float n_event_contamination[nEG][nStrict][nVtx+1];
  ...
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    ...
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      ...
      n_evts_trigBy_any[iEG][iBin] = 0;
      for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {
	n_evts_trigBy_spikes[iEG][iStrict][iBin] = 0;
	n_event_contamination[iEG][iStrict][iBin] = 0;
      }
    }
  }
  ...
  for(int iBin=0 ; iBin<nVtx ; iBin++) {
    ...
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      ...
      // Event contamination
      n_evts_trigBy_any[iEG][iBin] = (float)(h_evts_trigBy_any[iEG]->GetBinContent(iBin+1));
      n_evts_trigBy_any[iEG][nVtx] += n_evts_trigBy_any[iEG][iBin];

      for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {

	n_evts_trigBy_spikes[iEG][iStrict][iBin] = (float)(h_evts_trigBy_spikes[iEG][iStrict]->GetBinContent(iBin+1));
	n_evts_trigBy_spikes[iEG][iStrict][nVtx] += n_evts_trigBy_spikes[iEG][iStrict][iBin] ;

	if( n_evts_trigBy_any[iEG][iBin] != 0 )
	  n_event_contamination[iEG][iStrict][iBin] = n_evts_trigBy_spikes[iEG][iStrict][iBin] / n_evts_trigBy_any[iEG][iBin] ;
	else
	  n_event_contamination[iEG][iStrict][iBin] = -0.01;
      }
  ...
  for(int iEG=0 ; iEG<nEG ; iEG++) {
    ...
    // Event contamination
    for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {
      if(n_evts_trigBy_any[iEG][nVtx] != 0 )
	n_event_contamination[iEG][iStrict][nVtx] = n_evts_trigBy_spikes[iEG][iStrict][nVtx] / n_evts_trigBy_any[iEG][nVtx] ;
      else
	n_event_contamination[iEG][iStrict][nVtx] = -0.01 ;
    }
  ...
  // Event contamination
  outlog << endl << endl << "n_evts_trigBy_any : " << endl;
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    outlog << " iVtx=" << iBin << " : ";
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outlog << n_evts_trigBy_any[iEG][iBin] << " | " ;
    }
    outlog << endl;
  }
  ...
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    outlog << " iVtx=" << iBin << " : ";
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outlog << n_evts_trigBy_spikes[iEG][0][iBin] << " | " ;
    }
    outlog << endl;
  }
  ...
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    outlog << " iVtx=" << iBin << " : ";
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outlog << n_event_contamination[iEG][0][iBin] << " | " ;
    }
    outlog << endl;
  }  
  ...
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    outlog << " iVtx=" << iBin << " : ";
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outlog << n_evts_trigBy_spikes[iEG][1][iBin] << " | " ;
    }
    outlog << endl;
  }
  ...
  for(int iBin=0 ; iBin<nVtx+1 ; iBin++) {
    outlog << " iVtx=" << iBin << " : ";
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      outlog << n_event_contamination[iEG][1][iBin] << " | " ;
    }
    outlog << endl;
  }  
  ...
  TGraph * g_evt_contam[nEG][nStrict];
  for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {
    for(int iEG=0 ; iEG<nEG ; iEG++) {
      g_evt_contam[iEG][iStrict] = new TGraph( nVtx+1, x_vtx, n_event_contamination[iEG][iStrict] );
      g_evt_contam[iEG][iStrict]->SetName("Event "+strict_name[iStrict]+" contamination (EG"+trigname[iEG]+")");
      g_evt_contam[iEG][iStrict]->SetTitle("Event "+strict_name[iStrict]+" contamination (EG"+trigname[iEG]+")");
    }
  }
  ...
  for(int iEG=0 ; iEG<nEG ; iEG++) {
    ...
    h_evts_trigBy_any[iEG]->Write();
    h_evts_trigBy_any_sum[iEG]->Write();

    for(int iStrict=0 ; iStrict<nStrict ; iStrict++) {
      h_evts_trigBy_spikes[iEG][iStrict]->Write();
      h_evts_trigBy_spikes_sum[iEG][iStrict]->Write();
      g_evt_contam[iEG][iStrict]->Write();
    }
  }
  outplot->Close();
...
int spikes(int nEntries=-1, int iHalf=0, int nHalf=1, int sev_user=4, TString hlt_sel="",
	   TString dirOut="/home/llr/cms/ndaci/SKWork/macro/skEfficiency/Spikes2012/HPU/Run198603/", int run_sel=198603,
	   TString graph_style="A*", int marker_style=1, TString data="Spikes_2012_HPU", TString data_sum="high_PU", float spikeCut=8., bool debug=false)
{
  ...
  if(data=="Spikes_2012_HPU") {
    listTrees = list_spikes_2012_HPU();
  }
  else {
  ...
  TreeToHistos(myChain, data_sum, dirOut, outlog, nEntries, halftag, spikeCut, debug, graph_style,sev_user,hlt_sel,run_sel);
