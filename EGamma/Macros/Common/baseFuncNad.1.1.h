...
void fireL1(int L1noniso, int L1iso, vector<int> & firedEG, bool data2011) {
  
  const int n=10;
  int EG[n];
  int EG_2010[n] = {0,2,4,10,16,20,24,30,40,60};
  int EG_2011[n] = {0,1,2,5,8,10,12,15,20,30};
...
void globalFireL1(double eleRCT_eT, int noniso, int iso, int nonisoM, int isoM, 
		  vector<int> & firedEG_N, vector<int> & firedEG_M, bool data2011) {

  if( eleRCT_eT > 2 ) {// interesting regions <=> et > 2GeV
    fireL1( noniso , iso , firedEG_N, data2011 );
    fireL1( nonisoM , isoM , firedEG_M, data2011 );    
  }
}

void globalFireL1_Normal(double eleRCT_eT, int noniso, int iso,
			 vector<int> & firedEG_N, bool data2011) {
...
bool HLT_IdIso(TString asked,
	       double eT, double eta, double etaEle, double ele_tkSumPt_dr03, double ele_ecalRecHitSumEt_dr03, 
	      ...
	      double ele_conv_dist, double ele_conv_dcot, double ele_fbrem, int ele_isConversion)
{
  ...
  double values[7] = { ele_he , ele_sigmaietaieta , ecalIsoRel03 , hcalIsoRel03 , ele_deltaetain , ele_deltaphiin , trackIsoRel03 } ;

  if(asked=="HLT_Ele27_IdIso") {
    for(int i=0 ; i<7 ; i++) {
      if( etaEle<1.479 ) {
    ...
    cout << "PLEASE CHOOSE CORRECT ASKED FOR HLT_IDISO !!" << endl;
    return false;
  }
...
int whichCuts(double pt, double eta, double etaEle, double ele_tkSumPt_dr03, double ele_ecalRecHitSumEt_dr03, 
	      double ele_hcalDepth1TowerSumEt_dr03, double ele_hcalDepth2TowerSumEt_dr03, double ele_expected_inner_hits,
	      double ele_deltaphiin, double ele_deltaetain, double ele_he, double ele_sigmaietaieta,
	      double ele_conv_dist, double ele_conv_dcot, double ele_fbrem, int ele_isConversion)
{
  ...
  wp95 = VBTFcuts( "WP95",
		   pt,  eta,  etaEle,  ele_tkSumPt_dr03,  ele_ecalRecHitSumEt_dr03, 
